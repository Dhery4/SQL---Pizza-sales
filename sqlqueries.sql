...
-- here we rounded off to 2 digits

-- Q3) Identify the highest-prized pizza.
...
-- Q5) join the necessary tables to find the 
...
-- Q7)Determine the top 3 most ordered pizza types based on revenue.
...
-- Q8) Calculate the percentage contribution of 
-- each pizza type to total revenue.


SELECT 
    pizza_types.category, round(SUM(order_details.quantity * pizzas.price) / (select round(sum(order_details.quantity * pizzas.price),2) aas total sales 
